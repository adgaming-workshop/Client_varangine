...
void ge::createObject(std::string object_name){
	Client::getInstance()->data_storage.createObject(object_name);
}

void ge::deleteObject(std::string object_name){
	GameObject* obj = Client::getInstance()->data_storage.getObject(object_name);
	if (obj)
		obj->remove();
}

GameObject* ge::getObject(std::string object_name){
	return Client::getInstance()->data_storage.getObject(object_name);
}

void ge::setWindowSize(float x, float y){
	Client::getInstance()->render_controller.setWindowSize(x, y);
}

void ge::setWindowTitle(std::string title){
	Client::getInstance()->render_controller.setTitle(title);
}

void ge::setLayer(std::string object_name, int n){
	GameObject* obj = ge::getObject(object_name);
	if (n > Client::getInstance()->render_controller.getLayersCount())
		Client::getInstance()->render_controller.setLayersCount(n);
